 ...
 */
package au.gov.vic.delwp.gwc_to_slippy;
...
public class gwc_slippy {
    ...
    public static void main(String[] args) {
        ...
        Option acrgiscacheOp = new Option("sm", "slippy-cache", true, "Slippy Map Fromatted Tile Cache Output Location");
 ...
 //               .addOption(slippyOp)
        ...
        File thisJar = new File(gwc_slippy.class.getProtectionDomain().getCodeSource().getLocation().getPath());
        //System.out.println(thisJar.getParent());
        File logfileloc = new File(thisJar.getParent() + "\\log");
        File logfile = new File(thisJar.getParent() + "\\log\\gwc_to_slippy.log");
        File old_logfile = new File(thisJar.getParent() + "\\log\\old_gwc_to_slippy.log");
        ...
        log = Logger.getLogger(gwc_slippy.class);
        //long start = System.currentTimeMillis();
        String gwccache = cmd.getOptionValue("gwc-cache");
        String slippycache = cmd.getOptionValue("slippy-cache");
     ...
     //   slippy = cmd.hasOption("slippy");
        ...
        System.out.println("ArcGIS output - " + slippycache);
        ...
        File levelsFolder = new File(slippycache);
        levelsFolder.mkdir();
        String[] levelFolders = getLevelFolders(gwccache);
        for (String gwclevel : levelFolders) {
            int level = getSlippyMapLevelFolderName(gwclevel);
            ...
            for (String folder : folders) {
                ...
                ArrayList<String> files = getFiles(gwccache + File.separator + gwclevel + File.separator + folder, format);
                ...
                for (String file : files) {
                    ...
                    if (!new File(lvFolder.getPath() + File.separator + col).exists()) {
                        new File(lvFolder.getPath() + File.separator + col).mkdir();
                    }

                    //File dest = new File(lvFolder.getPath() + File.separator + rowFolder + File.separator + colFile);
                ...
                log.info("Complted: " + gwclevel + File.separator + folder);
            }
    ...
    public static int getSlippyMapLevelFolderName(String gwcLevelDir) {
